...
public class Grabber implements Grab {
    ...
    public void cfg() throws IOException {
        try (InputStream in = new FileInputStream("C:\\projects\\job4j_grabber\\src\\main\\resources\\app.properties")) {
            cfg.load(in);
        }
    }
    ...
    public static class GrabJob implements Job {
        ...
        public void execute(JobExecutionContext context) {
            ...
            parse.list("https://www.sql.ru/forum/job-offers")
                    .stream().filter(x -> x.getTitle().toLowerCase().contains("java"))
                    .filter(x -> !posts.contains(x))
                    .forEach(store::save);
        }
    }
